...
class PagoProveedorForm(FlaskForm):
    ...
    numero_comprobante = StringField(
        ...
        validators=[
            ...
            Length(
                max=50,
                message="El número de comprobante no debe exceder 50 caracteres.",
            ),
            Regexp(
                r"^[A-Za-z0-9\-\/]+$",
    ...
    observaciones = TextAreaField(
        ...
        validators=[
            Optional(),
            Length(
                max=300, message="Las observaciones no deben exceder 300 caracteres."
            ),
        ],
    )
    ...
    def validate_metodo_pago(self, field):
        if self.accion.data == "PAGADO" and not field.data:
            raise ValidationError(
                "Debes seleccionar un método de pago cuando la compra se marque como pagada."
            )

        if self.accion.data == "CANCELADO" and field.data:
            raise ValidationError(
                "No debes seleccionar método de pago si la compra será cancelada."
            )

    def validate_numero_comprobante(self, field):
        if (
            self.accion.data == "PAGADO"
            and self.metodo_pago.data == "TRANSFERENCIA"
            and not field.data.strip()
        ):
            raise ValidationError(
                "Debes capturar el número de comprobante para pagos por transferencia."
            )

        if self.accion.data == "CANCELADO" and field.data.strip():
            raise ValidationError(
                "No debes capturar comprobante si la compra será cancelada."
            )
    ...
    def validate_observaciones(self, field):
        ...
        if texto and len(texto) < 3:
            raise ValidationError(
                "Las observaciones deben tener al menos 3 caracteres si se capturan."
            )
