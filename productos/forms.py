...
class ProductoForm(FlaskForm):
    ...
    tiempo_produccion_minutos = IntegerField(
        ...
        validators=[
            Optional(),
            NumberRange(
                min=0, max=10080, message="El tiempo debe estar entre 0 y 10080 minutos"
            ),
        ],
    )
        ...
        if field.data:
            ...
            if re.search(r"\s{3,}", field.data):
                raise ValidationError(
                    "La descripción contiene demasiados espacios seguidos."
                )
    ...
    def validate_unidad_medida(self, field):
        if field.data:
            ...
            if field.data not in unidades_validas:
                raise ValidationError(
                    "Selecciona o escribe una unidad de medida válida."
                )
