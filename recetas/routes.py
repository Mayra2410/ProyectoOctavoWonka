...
def gestionar_ingredientes(id_receta):
    receta = Receta.query.get_or_404(id_receta)
    materias = (
        MateriasPrimas.query.filter_by(activo=True)
    ...
    if request.method == "POST":
        ...
        if not materia_prima_id:
            ...
            return redirect(
                url_for("recetas.gestionar_ingredientes", id_receta=id_receta)
            )
        ...
        if cantidad_necesaria <= 0:
            flash(
                "La cantidad debe ser un número mayor a 0. No se permiten valores negativos.",
                "danger",
            )
            return redirect(
                url_for("recetas.gestionar_ingredientes", id_receta=id_receta)
        ...
        existente = RecetaDetalle.query.filter_by(
            receta_id=id_receta, materia_prima_id=materia_prima_id
        ).first()

        if existente:
            flash(
                f'La materia prima "{materia.nombre}" ya está agregada a esta receta.',
                "warning",
            )
            return redirect(
                url_for("recetas.gestionar_ingredientes", id_receta=id_receta)
        ...
        try:
            ...
            db.session.rollback()
            flash(
                "Ocurrió un error al agregar el ingrediente a la base de datos.",
                "danger",
            )
