...
def recuperar_password():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            msg.body = (
                f"Tu codigo de seguridad para el cambio de contrasena es: {codigo}"
            )
            mail.send(msg)

            flash(
                "Hemos enviado un codigo a tu correo para autorizar el cambio.", "info"
            )
            return redirect(
                url_for("recuperarContrasenia.verificar_recuperacion", email=email)
            )
        else:
